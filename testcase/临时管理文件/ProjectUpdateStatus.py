...
api='api/Project/{0}/Status'
sheet_name = "ProjectUpdateStatus"

excel = ReadExcl.Xlrd()

@ddt.ddt
class ProjectUpdateStatus(unittest.TestCase): 
    @ddt.data(*excel.get_xls_next(sheet_name))
    def test_ProjectUpdateStatus(self,data):
        ...
        if r.status_code==expected_code:
            ...
            self.assertEqual(projectdetails['status'],status,case_describe + ",接口：{0}".format(api))
        else:
